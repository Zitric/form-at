...
**`packages/data`** holds what both apps genuinely share: the sets catalogue and its types (`sets.ts`), the DJ and event rosters (`djs.ts`, `events.ts` — plain hand-maintained arrays, not D1-backed; see [`TECH_DEBT.md`](TECH_DEBT.md) item 24), the per-set analytics query (`set-stats.ts`), and Web Push sending (`webPush.ts`). A set's `djId`/`eventId` are foreign keys into the DJ/event arrays' ids, picked from a dropdown in the admin upload/edit forms rather than inferred from free text. Queries used by only one app deliberately stay in that app — the admin dashboard's aggregate queries live in `apps/admin/app/data/admin-stats.ts` and have never had a second consumer, so they were never promoted.
