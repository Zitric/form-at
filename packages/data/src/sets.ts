...
// The catalogue's authoritative store is a D1 `sets` table, not a
// hand-maintained literal. `sets.generated.ts` (imported above) is a
// build-time snapshot regenerated from that table by
...
// and every CI job keep working with zero Cloudflare credentials. Only
// `deploy.yml`'s `deploy` job (apps/web) regenerates it live immediately
// before the production build; nothing else touches it.
//
