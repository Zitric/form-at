...
- [ ] **#8 — Full PWA (install + share routing + offline + polish)**
  ...
  ### Phase 2 — Shared links open the PWA, not the in-app browser
  ...
  ### Phase 3 — Offline caching (the original #8)
  ...
  ### Phase 4 — Polish
  - Per-device splash screens (iOS especially).
  - Update notification when a new deploy is live ("new build — tap to reload").
  - iOS heartbeat so the PWA doesn't get evicted by the 14-day storage cleanup.
  - "Manage offline sets" view (storage used, per-set remove).
